...
def mobilenet_v3_small(num_classes, *, pretrained=True, use_gpu=False):
    weights = None
    if pretrained:
        weights = torchvision.models.MobileNet_V3_Small_Weights.IMAGENET1K_V1

    model = torchvision.models.mobilenet_v3_small(weights=weights)
    model = _tweak_mobilenet(model, num_classes, use_gpu)
...
def mobilenet_v3_large(num_classes, *, pretrained=True, use_gpu=False):
    weights = None
    if pretrained:
